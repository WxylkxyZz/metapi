...
### 信任反向代理与 `TRUSTED_PROXY`

默认情况下 Metapi **不信任** `X-Forwarded-For` 等代理头——`request.ip` 永远是直连对端的真实 socket 地址，外部客户端无法通过伪造代理头绕过 `ADMIN_IP_ALLOWLIST` 或命中「本机回路」才放行的内部桥接（如模型测活强制通道）。

如果你在 Metapi 前面部署了反向代理（Nginx / Caddy / 云负载均衡），并需要让 Metapi 正确识别真实客户端 IP（例如用于管理端 IP 白名单、限流），必须显式通过 `TRUSTED_PROXY` 声明代理的来源 IP/网段：

```bash
# 单台反向代理
TRUSTED_PROXY=10.0.0.8

# 多个代理或网段（逗号分隔，支持 CIDR）
TRUSTED_PROXY=10.0.0.8,192.168.1.10,10.0.0.0/24
```

设置后，Metapi 会：
- 仅在**直连对端**是已声明代理时，才采信 `X-Forwarded-For`；
- 从右向左遍历代理头，跳过已声明的代理 IP，取第一个非代理 IP 作为真实客户端 IP（标准 `proxy-addr` 语义）。

> [!WARNING]
> 只有当你确认反向代理是唯一直连方、且外来请求不可能伪造该来源 IP 时才设置 `TRUSTED_PROXY`。声明错误的网段（如 `0.0.0.0/0`）会重新打开代理头伪造面。直接在公网暴露的部署**不要**设置，保持默认即可。

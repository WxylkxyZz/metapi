...
const zhToEn: Record<string, string> = {
  ...
  '指标口径': 'Metric Notes',
  'Canopy 将多个上游兼容供应商聚合为统一的 OpenAI / Claude 下游兼容入口。': 'Canopy aggregates multiple upstream compatible providers into a unified OpenAI / Claude compatible downstream endpoint.',
  ...
  'Webhook / Bark / Telegram，余额不足及时提醒': 'Webhook / Bark / Telegram — get notified when balance is low',
  ...
  '项目链接': 'Project Links',
  '数据与隐私': 'Data & Privacy',
  'Canopy 完全自托管，所有数据（账号、令牌、路由、日志）均存储在本地 SQLite 数据库中，不会向任何第三方发送数据。代理请求仅在你的服务器与上游站点之间直连传输。': 'Canopy is fully self-hosted. All data (accounts, tokens, routes, logs) is stored in a local SQLite database and never sent to any third party. Proxy requests travel directly between your server and upstream sites.',
};

...
describe('desktop navigation guard', () => {
  ...
  it('routes cross-origin popup links to the default browser', () => {
    expect(resolveDesktopNavigationAction('https://github.com/WxylkxyZz/canopy', appUrl)).toBe('deny');
  });

  it('keeps same-origin popup links inside the desktop app', () => {
    expect(resolveDesktopNavigationAction('/routes', appUrl)).toBe('allow');
  ...
  it('does not block same-origin navigations', () => {
    ...
    harness.getNavigateHandler()({ preventDefault }, 'http://127.0.0.1:5173/routes');
